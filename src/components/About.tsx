    ...
    <section id="about" className="relative mt-36 sm:mt-44 md:mt-48 lg:mt-56 py-24 px-4 md:px-8 max-w-7xl mx-auto overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="flex flex-col items-start text-left mb-16">
          <span className="text-xs font-mono text-blue-400 font-bold uppercase tracking-widest mb-2">// INSIGHTS</span>
          <h2 className="text-3xl md:text-5xl font-black tracking-tight text-white">About My Creative Journey</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="col-span-1 lg:col-span-7 flex flex-col items-start text-left">
            ...
            <div className="flex flex-wrap gap-2.5 mb-10">
              {["Graphic Designer", "Video Editor", "Meta Marketer", "Generative AI Tools"].map((skill, i) => (
                <span 
                  key={i} 
                  className="px-3.5 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-300 text-xs font-bold tracking-wide shadow-sm"
                >
                  {skill}
                </span>
              ))}
          ...
          <div className="col-span-1 lg:col-span-5 bg-gray-900/50 backdrop-blur-md border border-gray-800/40 rounded-3xl p-6 sm:p-8 shadow-2xl">
            {/* Tab selection buttons */}
            <div className="flex gap-2 bg-gray-950/60 p-1.5 rounded-2xl mb-8 border border-gray-800/20">
            ...
            <div className="min-h-[250px] flex flex-col justify-start">
              {activeTab === 'facts' && (
                ...
                >
                  <h4 className="text-lg font-bold text-white mb-2">Quick Facts</h4>
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-gray-300">{PERSONAL_INFO.location}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-gray-300">Available for select contracts</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      <p className="text-[10px] uppercase font-bold text-gray-500">Core Focus</p>
                      <p className="text-sm font-semibold text-gray-300">Visual Identities & Motion Assets</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      <p className="text-[10px] uppercase font-bold text-gray-500">Timezone</p>
                      <p className="text-sm font-semibold text-gray-300">GMT+6 (BST / Dhaka)</p>
                    </div>
                  </div>
                </motion.div>
              )}
              ...
              {activeTab === 'education' && (
                ...
                >
                  <h4 className="text-lg font-bold text-white mb-2">Academic Highlights</h4>
                  
                  {profile.education.map((edu, idx) => (
                    <div key={idx} className="relative border-l border-gray-800 pl-5 ml-2.5">
                      ...
                      <div className="flex justify-between items-start flex-wrap gap-2">
                        <p className="text-xs text-blue-400 font-mono font-bold tracking-wider">{edu.year}</p>
                        <span className="text-[10px] font-mono bg-blue-500/10 border border-blue-500/20 text-blue-300 px-1.5 py-0.5 rounded">Result: {edu.result}</span>
                      </div>
                      <h5 className="font-bold text-white text-sm mt-1">{edu.degree}</h5>
                      <p className="text-xs text-gray-400 mt-0.5">{edu.institute}</p>
                    </div>
                  ))}
                </motion.div>
              )}
...
function CreativeTimeline() {
  ...
  return (
    <div ref={containerRef} className="mt-24 pt-10 border-t border-gray-800/20">
      <div className="flex flex-col items-start text-left mb-14">
        <span className="text-xs font-mono text-blue-400 font-bold uppercase tracking-widest mb-2">// MILESTONES</span>
        <h3 className="text-xl sm:text-2xl font-bold text-white">My Creative Journey</h3>
      </div>

      <div className="relative border-l border-gray-800/60 ml-3 md:ml-4 space-y-12 text-left">
        {milestones.map((milestone, idx) => (
          ...
          >
            {/* Pulsing point icon */}
            <div className="absolute -left-[17px] top-1 h-8 w-8 rounded-lg bg-gray-950 border border-gray-800 flex items-center justify-center group-hover:border-blue-500/50 group-hover:shadow-[0_0_15px_rgba(59,130,246,0.3)] transition-all duration-300">
              {milestone.icon}
            </div>

            {/* Date Badge */}
            <span className="inline-block px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 font-mono text-[10px] font-black uppercase tracking-wider mb-2.5">
              {milestone.year}
            </span>

            {/* Title */}
            <h4 className="text-sm sm:text-base font-extrabold text-white mb-2 group-hover:text-blue-300 transition-colors">
              {milestone.title}
            </h4>
...
function StatCounter({ value, label, suffix }: { value: number; label: string; suffix: string; key?: any }) {
  ...
  return (
    <div 
      ref={containerRef}
      className="flex flex-col items-center sm:items-start p-4.5 bg-gray-900/40 dark:bg-gray-900/40 light:bg-gray-100 border border-gray-800/20 rounded-2xl text-center sm:text-left shadow-lg select-none"
    >
      <span className="text-3xl sm:text-4xl font-extrabold text-blue-500 tracking-tight tabular-nums">
        {count}{suffix}
      </span>
      <span className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-gray-500 mt-2 leading-tight">
        {label}
      </span>
    </div>
  );
